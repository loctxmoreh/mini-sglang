...
inline const char *cudaGetErrorString(hipError_t e) {
  return hipGetErrorString(e);
}
inline hipError_t cudaGetLastError() { return hipGetLastError(); }
inline hipError_t cudaFuncSetAttribute(const void *f, hipFuncAttribute a,
                                       int v) {
...
inline constexpr hipMemcpyKind cudaMemcpyDeviceToDevice =
...
#ifdef __HIP__
    // hipcc has no cudaLaunchKernelEx analog usable for our PDL-augmented
    // launch path. Use the standard <<<>>> triple-chevron syntax instead.
...
#ifndef __HIP__
    // cudaLaunchAttributeProgrammaticStreamSerialization is sm90+ only and has
    // no HIP equivalent; on HIP this becomes a no-op.
    if (use_pdl) {
